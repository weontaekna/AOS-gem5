...
if [[ $SCHEME == "AOS" ]]
then
    OUTPUT_DIR=$GEM5_PATH/output-AOS-compress-8MB/SPEC-$BENCHMARK
    mkdir $OUTPUT_DIR
    SCRIPT_IN=$GEM5_PATH/configs/example/spec06_config_inst.py
elif [[ $SCHEME == "PA" ]]
then
    OUTPUT_DIR=$GEM5_PATH/output-PA-compress-8MB/SPEC-$BENCHMARK
    mkdir $OUTPUT_DIR
...
then
    ...
    OUTPUT_DIR=$GEM5_PATH/output-compress-8MB/SPEC-$BENCHMARK
    mkdir $OUTPUT_DIR
