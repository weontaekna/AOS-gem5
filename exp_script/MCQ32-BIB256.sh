...
SPEC_PATH=/workspace/benchmarks/spec2006_arm
if [ -z ${GEM5_PATH+x} ];
then
...
if [[ $SCHEME == "AOS" ]]
then
    OUTPUT_DIR=$GEM5_PATH/output-AOS-MCQ32-BIB256/SPEC-$BENCHMARK
    mkdir $OUTPUT_DIR
    SCRIPT_IN=$GEM5_PATH/configs/example/spec06_config_inst.py
elif [[ $SCHEME == "PA" ]]
then
    OUTPUT_DIR=$GEM5_PATH/output-PA/SPEC-$BENCHMARK
    SCRIPT_IN=$GEM5_PATH/configs/example/spec06_config_pa.py
elif [[ $SCHEME == "AOS32" ]]
then
    SCHEME=AOS
    OUTPUT_DIR=$GEM5_PATH/output-AOS32/SPEC-$BENCHMARK
    SCRIPT_IN=$GEM5_PATH/configs/example/spec06_config_inst.py
else
...
#RUN_DIR=$SPEC_PATH/benchspec/CPU2006/$BENCHMARK_CODE/run/run_base_ref_arm_64_gcc.0000
...
$GEM5_PATH/build/ARM_MESI_Two_Level/gem5.opt \
  ...
  --cpu-type=DerivO3CPU --scheme=$SCHEME --btNumWays=1 --MCQEntries=32 --BIBEntries=256 \
