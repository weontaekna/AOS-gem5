    ...
    while (store_it != mchkQueue.begin()) {
        ...
        if (iter->PAC == store_it->PAC) {
            ...
            if (iter->state == ST1) {
                ...
                //    DPRINTF(MCQUnit, "Passed occupancyCheck by data forwarding! "
                //    "bndAddr: %lu lbndData: %lu ubndData: %lu count: %u "
                //    "PAC: %lu [sn:%lli]\n", iter->bndAddr, lbndData, ubndData, iter->count,
                //    iter->PAC, iter->inst_org->seqNum);
                ...
                //}
                //} else {
                //    DPRINTF(MCQCheck, "Failed occupancyCheck by data "
                //            "forwarding! (%s) bndAddr: %lu lbndData: %lu ubndData: %lu count: %u "
                //            "PAC: %lu [sn:%lli]\n",
