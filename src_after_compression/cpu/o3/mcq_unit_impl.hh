...
MCQUnit<Impl>::writebackStores()
{
    ...
    while (storesToWB > 0 &&
        ...
        if (!iter->instruction()->isBndStore()) {
            iter++;
            continue;
        }
        ...
        //Addr addr = iter->bndAddr + (iter->subIndex * 16);
        Request::Flags flags = TLB::MustBeOne|2|TLB::AllowUnaligned/* TODO */;
        ...
        if (!req->isSent()) {
            ...
            while (load_it != mchkQueue.end()) {
                ...
                if (load_it->PAC == iter->PAC &&
                    ...
                    if (load_it->instruction()->isBndStore())
    ...
    for (auto& x : mchkQueue) {
        ...
        if (x.state == ST5) {
            ...
            if (iter == mchkQueue.begin()) {
                ...
                if (iter->needUpdate && (iter->instruction()->isBndStore() ||
                    iter->isSigned)) {
...
MCQUnit<Impl>::completeDataAccess(PacketPtr pkt)
{
    ...
    if (!inst->isSquashed()) {
        ...
        for (int i=0; i<4; i++) {
            //Addr lbndData = *(pkt->getPtr<uint64_t>() + (2*i));
              ...
              case ST1:
                if (inst->isNeonLoad() && neonBoundCheck(addr, effSize, lbndData, ubndData)) {
                    ...
                    if (iter->count != 0) {
                        ...
                        DPRINTF(MCQUnit, "Passed neonBoundCheck! bndAddr: %lu addr: %lu effSize: %u "
                                "neon_lbndData: %lu neon_ubndData: %lu index: %lu count: %u PAC: %lu Hash: %lu [sn:%lli]\n",
                                iter->bndAddr, addr, effSize, neon_lbndData, neon_ubndData, iter->index, iter->count,
                                iter->PAC, iter->hash, iter->inst_org->seqNum);
                        iter->inst_org->setValidated(true); // signal to ROB
                    ...
                    if (iter->count != 0) {
                        ...
                        DPRINTF(MCQUnit, "Passed boundCheck! bndAddr: %lu addr: %lu effSize: %u "
                                "lbndData: %lu ubndData: %lu index: %lu count: %u PAC: %lu Hash: %lu [sn:%lli]\n",
                                iter->bndAddr, addr, effSize, lbndData, ubndData, iter->index, iter->count,
                                iter->PAC, iter->hash, iter->inst_org->seqNum);
                        iter->inst_org->setValidated(true); // signal to ROB
        ...
        if (!succeed) {
            //Addr lbndData = *(pkt->getPtr<uint64_t>());
            ...
            switch (iter->state) {
              case ST1:
                if (iter->needReplay) {
                    ...
                    DPRINTF(MCQCheck, "Failed boundCheck! bndAddr: %lu addr: %lu effSize: %u "
                            "lbndData: %lu ubndData: %lu index: %lu count: %u PAC: %lu Hash: %lu [sn:%lli]\n",
                            iter->bndAddr, addr, effSize, lbndData, ubndData, iter->index, iter->count,
                            iter->PAC, iter->hash, iter->inst_org->seqNum);
                    iter->state = ST4;
                    ...
                    //printf("boundCheck! addr: %lu effSize: %u lbndData: %lu ubndData: %lu\n",
...
MCQUnit<Impl>::setMCQEntry(typename MChkQueue::iterator& iter)
{
    ...
    if (inst->isBndstr()) {
        ...
        //memcpy((iter->data())+8, &iter->ubndData, 8);
...
MCQUnit<Impl>::neonBoundCheck(Addr addr, uint8_t effSize, Addr lbndData, Addr ubndData)
{
    ...
    //    printf("boundCheck! addr: %lu effSize: %u neon_lbndData: %lu neon_ubndData: %lu\n",
    //            addr, effSize, neon_lbndData, neon_ubndData);
    //}
...
MCQUnit<Impl>::execute()
{
    ...
    for (; inst_num < insts_to_execute; ++inst_num) {
        ...
        switch (iter->state) {
          case ST0:
            if (inst->isBndStore()) {
                DPRINTF(MCQUnit, "Bound store inst encountered PC %s, idx:%i [sn:%lli]\n",
          ...
          case ST4:
            if (iter->needReplay) {
                ...
                iter->bndAddr = getBndAddr(iter->PAC,
                                    iter->index, iter->count);

                iter->state = inst->isBndStore() ? ST2 : ST1;
                readyInsts.push_back(inst);
            } else {
                iter->count++;
...
MCQUnit<Impl>::BWB::eviction()
{
    ...
    //printf("Evicted from BWB (CurSize:%lu), PAC: %lu Hash: %lu Index: %lu\n",
    //        m_list.size(), entry->PAC, entry->hash, entry->index);
...
MCQUnit<Impl>::BWB::getIndex(uint64_t PAC, uint64_t hash, bool isBndstr)
{
    ...
    if (isBndstr) {
        for (; it != m_list.end(); ++it) {
            ...
            //    if ((*it)->occupied)
            //        return ((*it)->index + 1) % mcqUnit->btNumWays;
            //    else 
            //        return (*it)->index;
            //}
        }
